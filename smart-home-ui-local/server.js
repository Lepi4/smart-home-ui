...
app.post('/api/levels/:id/activate', async (req,res)=>{ try{ const lp=requestProfileLevelContext(req); const data=await withTemporaryLevel(lp.profileId, lp.id, async()=>activateLevel(req.params.id)); updateActiveProfilePaths(); const levelId=sanitizeLevelId(req.params.id); try{ saveCurrentClientSettings(req, { activeProfileId: lp.profileId, activeLevelId: levelId, navigation: { profileId: lp.profileId, levelId } }); }catch(_){} const cid=sanitizeClientId(req.headers['x-client-id'] || ''); if(cid){ const prefs=getClientPrefs(cid); prefs.activeProfileId=lp.profileId; prefs.activeLevelId=levelId; prefs.navigation={...(prefs.navigation||{}), profileId:lp.profileId, levelId}; saveClientPrefs(cid, prefs, req); } res.json({ok:true, ...data, reloadRecommended:true}); }catch(e){ validationErrorResponse(req,res,e); } });
